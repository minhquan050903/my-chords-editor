  ...
  const handleImageUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (uploadedImages.length >= 20) {
        alert("You have reached the maximum limit of 25 images.");
        return;
      }
    ...
    if (boxRef.current) {
      toJpeg(boxRef.current, { backgroundColor: 'white' })
        .then((dataUrl) => {
          ...
          link.download = `mychord-${title}.jpeg`;
          link.href = dataUrl;
          link.click();
        })
    }
  ...
  return (
    ...
    <Box className="App" justifyContent="center">
      ...
      <Box id="chords" ref={boxRef} sx={{
        ...
        borderRadius: 1,
        height: 350 * Math.ceil(uploadedImages.length / 5)
        ...
        <GridContextProvider onChange={handleOnChange}>
          <GridDropZone
            ...
            boxesPerRow={5}
            rowHeight={300}
            style={{ height: 340 * Math.ceil(uploadedImages.length / 5) }}
          >
